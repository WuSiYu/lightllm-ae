...
def sample_requests(
    ...
    assert num_requests <= len(dataset), f"more reqs than dataset ({len(dataset)} max)"


    sampled_requests: List[Request] = []
    tmpl = '^&' * 8192
    for i in range(num_requests):
        l = dataset[i]['prompt_tokens']
        l_out = dataset[i]['completion_tokens']
...
async def send_request(
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        async with session.post(url, headers=headers, json=req_json, timeout=timeout) as response:
            ...
            try:
                ...
                tokenizer_debug = get_tokenizer(args.tokenizer, "slow")
                print(f"Error on req: {i}, chunks: {chunks}, prompt_len: {prompt_len}, e: {e}\n", end='')    # thread-safe print
                # print("req:", req_json)
                # sys.exit(1)
                return False
        ...
        if 'abort' in chunks[-1]:
            print(f"Server (SLA) abort: req #{i}, SLA: TTFT<={sla_ttft}, MTPOT<={sla_mtpot}, archived: {latencies[0]:.4f} and {max(latencies[1:] + [-1]):.4f} (s), input: {prompt_len}, decoded: {len(latencies)-1} tokens\n", end='')
            return False
    ...
    if latencies[0] > sla_ttft or max(latencies[1:]) > sla_mtpot:
        print(f"SLA violent: req #{i}, SLA: TTFT<={sla_ttft}, MTPOT<={sla_mtpot}, archived: {latencies[0]:.4f} and {max(latencies[1:]):.4f} (s), input: {prompt_len}, decoded: {len(latencies)} -1 tokens\n", end='')
        return False
    else:
