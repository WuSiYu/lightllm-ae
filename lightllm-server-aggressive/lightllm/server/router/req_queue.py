...
class FuturePastReqQueue(ReqQueue):
    ...
    def _can_add_new_req(self, req: Req, is_busy: bool):
        used_tokens = sum(a for a, b in self._cache_len_lists[0])
        need_max_token_nums = []
        ...
        ok_token_num = used_tokens < self.max_total_tokens * (1 - self.REVERSED) - self.cache_pause_reqs_used_tokens - self.prompt_cache_used_tokens \
            and used_tokens + req.get_tuple_tokens(False, 0, minimal_output_len_factor=1)[0] < self.max_total_tokens
        ok_req_num = len(self.cache_len_list) + self.cache_pause_reqs_num + self.prompt_cache_req_num <= self.running_max_req_size
